...
%\section[Novel computational tech\-niques in particle phy\-sics]{Novel computational  tech\-niques in particle phy\-sics}
\section[Particle physics: Novel computational techniques]{Particle physics: novel computational techniques}
...
The research focus of this group is the development of methods
and tools to push the precision of perturbative calculations and event 
simulations for collider physics. Thereby we enable more 
detailed comparisons between theoretical predictions and experimental measurements.
...
core of any experimental analysis performed at colliders.
...
of infrared-safe observable in comparison to data.
...
in the event simulations on the other hand. This will allow us ultimately to observe even the smallest
deviations from SM predictions and it opens up a crucial avenue towards observing new-physics phenomena.
...
processes~\cite{Monni:2019whf,Monni:2020nks}, generalized to $2\to 2$ reactions 
...
Expanding upon the previous advancements of the \minnlo{} method for $t \bar t$ production, 
...
general quark masses as well as different flavour-number schemes.
...
We are currently finalizing a second study on $b$-jet observables using our $b\bar b$ \minnlo{} simulation.
A preliminary comparison of our \minnlo{} predictions against ATLAS data is given in 
\fig{fig:bb}.
The developed generator also opens the possibility to study how the definition of the jet flavour (see for example \citere{Gauld:2022lem}) 
impacts theory simulations.
...
Experimental studies of charm production rates in $pp$, $pA$ and $AA$ collisions are carried out by the LHC experiments, and the interpretation of their data is relevant for our understanding of both nuclear and proton structure, as well as providing unique information on states of matter such as the quark gluon plasma.
Furthermore, charm hadron-hadron production is relevant for neutrino telescopes, such as IceCube, as knowledge of the rate of the prompt atmospheric neutrino flux (induced by charm production in the atmosphere) is essential for studies of neutrinos of cosmic origin.
...
By upgrading these predictions with NNLO accuracy for $c\bar c$ production using 
...
\end{Namen}
Higgs production in association with bottom quarks (\bbH{}) is a rare production mode within the SM at the LHC.
However, it is irreducible background in Higgs-pair searches and gets potentially enhanced in BSM scenarios. 
Precise predictions of this process are important not only for experimental comparisons but also serve as a theoretical laboratory for studying the effects of heavy quarks. 
In processes involving bottom quarks, predictions can be obtained using different approaches to treat mass effects. In the 5FS scheme, the bottom-quark component of the proton is perturbatively generated, enabling the resummation of large logarithmic contributions in the bottom mass. 
This approach treats the bottom quark as a massless parton, simplifying high-order calculations. In~\citere{Biello:2024vdh}, we presented the first NNLO+PS simulation of this process within the massless scheme. For a more complete description of bottom-quark kinematics, the 4FS can be considered, where two massive bottom quarks and the Higgs boson are produced in the hard scattering. 
While computationally more demanding, this latter approach retains all mass effects order by order in perturbative QCD. Using the \minnlo{} framework, we calculated the NNLO corrections in the massive 
scheme and matched them with a parton shower simulation in \citere{Biello:2024pgo}.
At lower perturbative accuracy, strong tension has been observed between the predictions in the massless and massive schemes for \bbH{} production.
With the inclusion of NNLO corrections in both schemes, a better substantially agreement is found for two NNLO+PS predictions, as shown in~\fig{phenofig:bbH}. 

Since the two schemes capture complementary aspects of the bottom-quark dynamic, a generalized mass-variable flavour number scheme is desirable to achieve 
precise predictions across phase space. Using the method developed in \citere{Gauld:2021zmq}, we are investigating the isolation 
of mass power corrections within the massive scheme, aiming to incorporate them into the massless prediction. 
This would allow us to retain the benefits of the logarithmic resummation of the bottom mass in the massless scheme, 
while systematically accounting for missing power corrections in the bottom-quark mass.

\begin{figure}[phtb]
\begin{center}
\includegraphics[width=0.95\linewidth]{plots/bbH__ptHspectrum.pdf}
\caption{Transverse momentum of the Higgs boson in \bbH{} production in 5FS (blue, dotted) and 4FS (4FS, red, solid) at NNLO+PS, compared to NNLL+NNLO in 5FS (green, dashed).}
\label{phenofig:bbH}
\end{center}
\end{figure}
%
...
\caption{Different matching schemes for the dilepton mass in $WZ$ process.}
\label{fig:WZ}
\end{center}
\end{figure}
...
\subsubsection{Polarized NLO+PS predictions and quantum information}
\begin{Namen}
...
\citere{Pelliccioli:2023zpd} presents a calculation and simulation
...
energies, \citere{Grossi:2024jae} studies polarisation and
spin-correlation coefficients in diboson systems. It is shown that
...
analysis of diboson pairs at the LHC, either in inclusive electroweak
...
\caption{Top-mass effects in Higgs $p_T$ spectrum.}
...
%at the example of 
for the Higgs transverse-momentum ($p_T$)
...
novel full-theory results. This will be useful to provide well-motivated 
...
%
\subsubsection{Vector Boson Fusion Higgs production with decay to  bottom quarks}
\begin{Namen}
A. Behring, G. Zanderighi
\end{Namen}
Higgs production in vector boson fusion (VBF) is the second most common
production channel at the LHC. 
%This explains the considerable interest from
%both theorists and experimentalists in precise descriptions of this process.
The Higgs decays predominantly to a pair of bottom quarks, making the
combination of VBF with the $H \to b\bar{b}$ decay channel particularly 
interesting.
...
these descriptions in \citere{Asteriadis:2024nbg} in the narrow-width
approximation.
%
...
In \citere{Asteriadis:2024nbg} we managed to trace this back to an interplay
...
%
Given the expertise with parton-shower-matched calculation in the group at MPP, it was natural to join forces and combine the NNLO+PS calculation for $H \to b\bar{b}$ decays from \citere{Bizon:2019tfo} with the NNLO description for VBF from \citere{Asteriadis:2021gpd}. This project is currently ongoing.

\subsubsection{Higgs production in association \mbox{with a top-antitop quark at NNLO}}
\begin{Namen}
 J. Mazzitelli 
\end{Namen}

The associated production of a Higgs boson with a top-antitop quark pair is a crucial process at the LHC since it allows for a direct measurement of the top-quark Yukawa coupling. Ref.\,\cite{Catani:2022mfv} presents the computation of the radiative corrections to this process at NNLO. This is the very first computation for a $2 \to 3$  process with massive colored particles at this perturbative order. A soft Higgs boson approximation for loop amplitudes is developed, which enables one to reliably quantify the impact of the yet unknown two-loop contribution. It is found that in LHC collisions at the center-of-mass energy $s=13$\,TeV, the NNLO corrections increase the NLO result for the total cross section by about 4\% and lead to a significant reduction of perturbative uncertainties.

\subsubsection{\mbox{Di-Higgs production at NNLO+PS}}
\begin{Namen}
...
corrections is quite cumbersome, and so far limited to NLO QCD in the full theory. However, by combining
...
The impact of such terms has been computed via all-order NLP resummed corrections and applied to the case of the SM Higgs boson \cite{Das:2024pac} as well as a (BSM) pseudo-scalar Higgs boson \cite{Ravindran:2023qae}. In both cases, extensive phenomenological analyses to study the impact of NLP resummation on Higgs production at the LHC have been presented.
...
%
The continued development of theoretical tools and methods is therefore critical to further improve the modelling of the available theory predictions. This remains an active research activity of our group. %, for example: the efficient evaluation of Feynman integrals, towards automation of or going beyond NNLO QCD, the inclusion of quark mass and flavour effects.  
...
Feynman integrals are central objects in the perturbative descriptions of quantum field theories, and their evaluation is also one of the main sources of difficulties in our quest for even more precise theory predictions.
Intriguingly, various kinds of special functions in mathematics appear naturally in the computation of Feynman integrals.
In \citere{Liu:2023jkr}, a recipe for efficiently evaluating Feynman integrals through the auxiliary mass flow method and differential equations in the kinematic variable space is proposed.
...
\begin{Namen}
\mbox{G. Pelliccioli, A. Ratti, C. Signorile-Signorile}
\end{Namen}
% To achieve reliable theoretical predictions for key QCD processes, it is essential to explore higher orders in perturbation theory. This requires dealing with infrared (IR) singularities that arise from both virtual and real corrections. 
...
As a result, numerous NNLO subtraction schemes continue to be developed and refined. In \citere{Bertolotti:2022aih}, the \emph{local analytic sector subtraction} method was employed to demonstrate the cancellation of IR singularities for arbitrary processes at $e^+e^-$ colliders. This approach yields compact and fully analytic expressions, making it well-suited for direct numerical implementation. 
%To date, it remains the only fully local and analytic subtraction scheme capable of achieving this level of generality for lepton scattering at NNLO in QCD.
More recently, \citere{Magnea:2024jqg} marked the first steps toward extending this framework to N${}^3$LO, along with some complementary studies based on the factorisation properties of gauge amplitudes that can lead to interesting insights on the general organisation of subtraction schemes at all orders in perturbation theory.
In parallel, addressing the growing demand for precision predictions in multi-parton processes at hadron colliders, the \emph{nested soft-collinear subtraction} scheme has been revisited to cope with $pp$ scatterings into arbitrary number of coloured and colourless partons. Notably, in \citere{Devoto:2023rpv} and~\cite{Devoto:2025kin}, 
...
%
\subsubsection{Soft function at N$^3$LO}
\begin{Namen}
M. Delto, C. Wang
\end{Namen}
%
...
In \citere{Baranowski:2024ene,Baranowski:2024vxg,Baranowski:2024ysi}, we completed the calculation of the last missing ingredient of the zero-jettiness slicing method for color-singlet production at $\text{N}^{3}\text{LO}$, marking our first step towards the $N$-jettiness slicing method for general colorful final states.
...
\subsubsection{$Q_{1,2}$--$Q_7$ interference contributions to $b \to s \gamma$ at ${\mathcal{O}}(\alpha_s^2)$}
\begin{Namen}
M. Niggetiedt
\end{Namen}
%
...
In order to match the experimental accuracy, NLO electroweak as well as exact NNLO QCD %(${\mathcal{O}}(\alpha_s^2)$) 
...
%
\subsubsection{Z+c-jet at NNLO in QCD}
\begin{Namen}
R. Gauld
\end{Namen}
%
Another avenue of development is fixed-order calculations which take into account the full flavour information of scattering processes, propagated also at the level of (integrated) subtraction terms. Such a development was required for the computation of the process $pp \to Z + c-${\rm jet}, as presented in~\citere{Gauld:2023zlv}. 
%
...
\subsubsection{Top-mass renormalization in $t\bar t$ at NNLO}
\begin{Namen}
J. Mazzitelli
\end{Namen}
...
$\overline{\rm MS}$ schemes are presented. The study shows that the differences between
the two schemes are sizable, but the difference is largely reduced
...
series in the $\overline{\rm MS}$ scheme is much better than in the OS one.
%
\printbibliography[heading=subbibliography]
\end{refsection}
...
enable a competitive extraction of the strong-coupling constant by considering event shapes in
$N$-jet final states.
...
we have adapted the POWHEG framework to deal with leptons in the initial state and derived
...
\subsubsection{Off-shell NLO predictions in $t\bar t(+X)$ processes}
\begin{Namen}
G. Pelliccioli
\end{Namen}
...
uncertainty on distinct fronts.
...
``world'' average is taken as reference input in all QCD
calculation. A decision on which extraction to include and how to
combine them is therefore assessed with greatest care.
...
%
In \citere{Nason:2023asn} only ALEPH data at the $Z$-pole were
...
includes experimental uncertainties and the second one includes
...
In \citere{Wu:2023ntn}, we investigated several hidden-charm tetra-quark systems using QCD sum rules and provided a possible explanation of the observed signals.
...
%In both $e\nu_e$ and BSM cases, we compared our PDF results with MadGraph5$\_$aMC simulations of the full leading order processes (e.g. $\mu^-\mu^+\to e^-\bar{\nu}_eW^+$) and we identified generation cuts for which the two approaches are compatible. 
%A rigorous comparison between the two approaches needs further theoretical developments in the description of electroweak radiation at high energies and the improvement of PDFs with higher-order corrections.
%
\printbibliography[heading=subbibliography]
\end{refsection}
...
\begin{Namen}
U. Haisch, J. Linder, G. Pelliccioli, G. Zanderighi
\end{Namen}
%
...
%
\subsubsection{Matrix Element Method in Powheg and \minnlo{}}
\begin{Namen}
...
relies on the matrix elements for the given event kinematics to define the observable. This allows us
...
as a final goal, NNLO+PS predictions based on \minnlo{}. Our aim is to achieve a more reliable
